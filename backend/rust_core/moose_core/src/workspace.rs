...
pub struct SharedWorkspace {
    inner: Arc<Mutex<SharedWorkspaceInner>>,
...
impl SharedWorkspace {
    ...
    fn new(db_path: Option<String>) -> PyResult<Self> {
        ...
        let conn = Connection::open(&db_path).map_err(WorkspaceError::from)?;
        conn.execute_batch("PRAGMA journal_mode=WAL; PRAGMA synchronous=NORMAL;").map_err(WorkspaceError::from)?;
        Self::init_schema(&conn)?;
        Ok(Self { inner: Arc::new(Mutex::new(SharedWorkspaceInner { conn })), cache: Arc::new(DashMap::new()) })
    }
    ...
    fn query(&self, mission_id: String, agent_id: Option<String>, entry_type: Option<String>) -> PyResult<Vec<HashMap<String, String>>> {
        ...
        let mut stmt = inner.conn.prepare(&sql).map_err(WorkspaceError::from)?;
        let results: Vec<HashMap<String, String>> = match (agent_id.as_ref(), entry_type.as_ref()) {
            (Some(a), Some(t)) => stmt.query_map(params![mission_id, a, t], |row| {
                ...
                Ok(m)
            }).map_err(WorkspaceError::from)?.filter_map(|r| r.ok()).collect(),
    ...
    fn clear_mission(&self, mission_id: String) -> PyResult<u64> {
        ...
        let inner = self.inner.lock();
        let count = inner.conn.execute("DELETE FROM workspace_entries WHERE mission_id = ?1", params![mission_id]).map_err(WorkspaceError::from)?;
        Ok(count as u64)
    }

    fn count(&self) -> PyResult<u64> {
        let inner = self.inner.lock();
        let count: u64 = inner.conn.query_row("SELECT COUNT(*) FROM workspace_entries", [], |row| row.get(0)).map_err(WorkspaceError::from)?;
        Ok(count)
    }
    ...
    fn clear(&self) -> PyResult<()> {
        self.cache.clear();
        let inner = self.inner.lock();
        inner.conn.execute("DELETE FROM workspace_entries", []).map_err(WorkspaceError::from)?;
        Ok(())
    }
}
