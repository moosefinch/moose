"""
OutreachAgent — marketing and business development specialist.
Routes inference to Hermes 4 70B central engine with filtered outreach tools.
"""
...
from plugins.crm.prompts import OUTREACH_SYSTEM_PROMPT
from config import TOKEN_LIMITS, TEMPERATURE, MAX_TOOL_ROUNDS
...
class OutreachAgent(BaseAgent):
    ...
    async def run(self, message: AgentMessage, bus, workspace) -> Optional[AgentMessage]:
        """Execute an outreach task with tool-calling loop."""
        ...
        try:
            ...
            self.post_to_workspace(
                ...
                f"Outreach: {task_desc[:60]}",
                response_text,
                tags=["outreach", "campaign", "execution"],
            )
