"""
CoderAgent — Code generation, debugging, refactoring specialist.

Routes inference to Hermes 4 70B central engine with filtered code tools.
"""
...
class CoderAgent(BaseAgent):
    ...
    async def run(self, message: AgentMessage, bus, workspace) -> Optional[AgentMessage]:
        """Execute a code task with tool-calling loop."""
        ...
        try:
            ...
            self.post_to_workspace(
                ...
                f"Coder: {task_desc[:60]}",
                response_text,
                tags=["coder", "code", "execution"],
            )
